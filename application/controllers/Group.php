...
class Group extends CI_Controller
{
    ...
    // Mendapatkan daftar spesialis dari model
    $data['specialists'] = $this->User_model->getSpecialists();

    // Mendapatkan kontak psikiater
    $psikiater_contacts = $this->User_model->getPsikiaterContacts();
    $data['no_hp'] = $this->User_model->getPsikiaterPhoneNumbers();

    

    // Mengelompokkan kontak psikiater berdasarkan spesialis
    $data['psikiater_by_specialist'] = [];
    foreach ($psikiater_contacts as $contact) {
        $spesialis = $contact->spesialist;
        if (!isset($data['psikiater_by_specialist'][$spesialis])) {
            $data['psikiater_by_specialist'][$spesialis] = [];
        }
        $data['psikiater_by_specialist'][$spesialis][] = $contact;
    }

       
        $this->template->load('template/main_template', 'chat/group/spesialist_psikiater', $data);
    }

    // Controller: Group.php

public function kanselor() {
    $this->load->model('User_model');
    
    // Mendapatkan daftar spesialis dari model
    $data['specialists'] = $this->User_model->getSpecialistsK();

    // Mendapatkan kontak kanselor
    $kanselor_contacts = $this->User_model->getKanselorContacts();
    $data['no_hp'] = $this->User_model->getPsikiaterPhoneNumbers();

    // Mengelompokkan kontak kanselor berdasarkan spesialis
    $data['psikiater_by_specialist'] = [];
    foreach ($kanselor_contacts as $contact) {
        $spesialis = $contact->spesialist;
        if (!isset($data['psikiater_by_specialist'][$spesialis])) {
            $data['psikiater_by_specialist'][$spesialis] = [];
        }
    ...
    $this->template->load('template/main_template', 'chat/group/spesialist_kanselor', $data);
}
