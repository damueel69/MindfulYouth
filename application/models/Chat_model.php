...
class Chat_model extends CI_Model 
{
    ...
    public function getContactsForUser($user_id)
    {
        $this->db->select('users.id, users.username, users.is_logged_in');
        $this->db->from('chats_details');
        $this->db->join('users', 'users.id = chats_details.user_id');
        $this->db->where('chats_details.chat_id IN (SELECT chat_id FROM chats_details WHERE user_id = '.$user_id.')', NULL, FALSE);
